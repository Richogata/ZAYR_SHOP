...
export function HelixLoader({
  size = 45,
  speed = 2.5,
  color = "#c2410c", // Gold / amber theme matching ZAYR SHOP premium styling
}: HelixLoaderProps) {
  const parsedSize = typeof size === "number" ? `${size}px` : size;
  const parsedSpeed = typeof speed === "number" ? `${speed}s` : speed;
      ...
      <style dangerouslySetInnerHTML={{
        __html: `
          ...
          .helix-slice::after {
            ...
            flex-shrink: 0;
            animation: helix-orbit var(--uib-speed) linear infinite;
